...
export default async function ChapterPage({ params }) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      {locations.length > 0 && (
        <section className="mb-12 bg-slate-100 rounded-3xl overflow-hidden aspect-[16/10] relative border border-slate-200 shadow-inner">
          <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center">
            <div className="text-4xl mb-4">📍</div>
            <h2 className="text-lg font-bold text-slate-800 mb-2">地圖預覽（開發中）</h2>
            <p className="text-sm text-slate-500 max-w-xs">
              此冊包含 {locations.length} 個地點。未來將在此顯示 Google Maps 交互地圖。
            </p>
            <div className="mt-6 flex flex-wrap justify-center gap-2">
              {locations.map((loc, idx) => (
                <span key={idx} className="bg-white px-3 py-1 rounded-full text-xs font-medium shadow-sm border border-slate-100">
                  {loc.name}
                </span>
              ))}
            </div>
            ...
            <details key={idx} className="group border border-slate-200 rounded-2xl overflow-hidden bg-white shadow-sm transition-all hover:border-slate-300">
              <summary className="flex items-center justify-between p-6 cursor-pointer list-none">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="text-xl font-bold group-open:text-blue-600 transition-colors">
              ...
              <div className="px-6 pb-8 pt-2 border-t border-slate-50">
                {post.frontmatter.cover_image && (
                  <img 
                    ...
                    className="w-full aspect-video object-cover rounded-xl mb-6 bg-slate-50"
                  />
                )}
                ...
                {post.frontmatter.location_name && (
                  <div className="mt-8 pt-6 border-t border-slate-100 flex items-center justify-between gap-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="text-sm font-bold">{post.frontmatter.location_name}</div>
                    ...
                    <a 
                      ...
                      className="shrink-0 bg-slate-900 text-white px-4 py-2 rounded-full text-xs font-bold hover:bg-slate-800 transition-colors"
                    >
                      查看地圖
                    </a>
                  </div>
