...
## 變更紀錄
- 2026-03-23：修正 Page 詳細頁字體不一致（桌機 vs 手機）問題。根因為 `globals.css` 的 `body` 被硬編碼為 `Arial`，且 `/page/[slug]` 內文容器使用 `font-serif`，導致中文 fallback 與標題字體不同；已改為全站以 `--font-geist-sans` + CJK sans fallback 作為基底，並將 Page summary/內文/blockquote 裝飾引號統一改為 `font-sans`，讓文章文字在桌機與手機皆與標題字體體系一致。
